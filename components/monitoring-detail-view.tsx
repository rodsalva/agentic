...
function ScopedChat({ name }: { name: string }) {
  ...
  return (
    <div className="border-t border-gray-100">
      {messages.length > 0 && (
        <div className="px-6 py-4 space-y-3 max-h-40 overflow-y-auto [&::-webkit-scrollbar]:w-[2px] [&::-webkit-scrollbar-thumb]:bg-gray-200">
          {messages.map((m, i) => (
          ...
          {loading && <p className="text-sm text-gray-300">...</p>}
        </div>
      )}
      <div className="px-6 py-4 flex items-center gap-3">
        <input
          ...
          placeholder={`Pergunte sobre este monitoramento...`}
